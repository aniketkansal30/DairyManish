...
export default function ProductsView({ products, onSave, onDelete, dbCats, setDbCats }) {
  const [form, setForm] = useState({
    name: "", category: "Sweets", price: "", cost: "",
    unit: "kg", hasVariation: false, halfPrice: "", fullPrice: "",
  });
  ...
  const del = async (id) => {
    if (window.confirm("Yeh product delete karna chahte ho?")) await onDelete(id);
  };
  ...
  const FormPanel = () => (
    <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #e5e0d8", padding: 20, position: mobile ? "relative" : "sticky", top: mobile ? "unset" : 80 }}>
      ...
      {[["Product Name", "name", "text", "Paneer"], ["Selling Price (₹)", "price", "number", "400"], ["Cost Price (₹)", "cost", "number", "280"]].map(
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
          {["kg", "litre", "piece"].map((u) => (
            <button key={u} onClick={() => setForm((p) => ({ ...p, unit: u }))} style={{ flex: 1, padding: "8px", borderRadius: 8, border: `1.5px solid ${form.unit === u ? "#1a1310" : "#e5e0d8"}`, background: form.unit === u ? "#1a1310" : "#fff", color: form.unit === u ? "#f59e0b" : "#4a3f35", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              {u}
            </button>
          ))}
        </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {(editing || mobile) && (
          <button onClick={() => { setEditing(null); setForm({ name: "", category: "Sweets", price: "", cost: "", unit: "kg", hasVariation: false, halfPrice: "", fullPrice: "" }); if (mobile) setShowForm(false); }}
        ...
        <button onClick={save} disabled={saving}
          style={{ flex: 1, padding: "11px", background: "#1a1310", color: "#f59e0b", border: "none", borderRadius: 10, fontWeight: 800, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: saving ? 0.7 : 1 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "320px 1fr", gap: 24, alignItems: "start" }}>
      ...
      <div style={{ maxHeight: mobile ? "unset" : "calc(100vh - 100px)", overflowY: mobile ? "unset" : "auto", paddingRight: mobile ? 0 : 4 }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 16, alignItems: "center" }}>
          ...
          {mobile && (
            <button onClick={() => { setEditing(null); setForm({ name: "", category: "Sweets", price: "", cost: "", unit: "kg", hasVariation: false, halfPrice: "", fullPrice: "" }); setShowForm(true); }}
              style={{ flexShrink: 0, padding: "10px 16px", background: "#1a1310", color: "#f59e0b", border: "none", borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #e5e0d8", overflow: "hidden" }}>
            ...
            {filtered.map((p, i) => (
              <div key={p.id} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr 80px", padding: "12px 16px", borderTop: i > 0 ? "1px solid #f0ebe4" : "none", alignItems: "center", fontSize: 13 }}>
                ...
                <span style={{ display: "inline-block", padding: "3px 8px", borderRadius: 20, background: (CAT_COLORS[p.category] || "#f59e0b") + "22", color: CAT_COLORS[p.category] || "#f59e0b", fontSize: 11, fontWeight: 700 }}>{p.category}</span>
                <span style={{ fontWeight: 700, color: "#2563eb" }}>₹{p.price}</span>
                <span style={{ color: "#ef4444" }}>₹{p.cost}</span>
                <span style={{ color: "#16a34a", fontWeight: 700 }}>
                  ₹{p.price - p.cost} <span style={{ fontSize: 10, color: "#8a7e6e" }}>({Math.round(((p.price - p.cost) / p.price) * 100)}%)</span>
                </span>
            ...
            {filtered.length === 0 && <div style={{ textAlign: "center", color: "#c9b9a8", padding: "30px 0", fontSize: 13 }}>No products found</div>}
          </div>
        )}
            ...
            {filtered.map((p) => (
              <div key={p.id} style={{ background: "#fff", borderRadius: 14, border: "1px solid #e5e0d8", padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: "#1a1310" }}>{p.name}</div>
                  <div style={{ fontSize: 11, color: "#8a7e6e", marginTop: 2 }}>{p.unit} · {p.category}</div>
                  <div style={{ display: "flex", gap: 10, marginTop: 6, alignItems: "center" }}>
                    <span style={{ fontSize: 14, fontWeight: 900, color: "#2563eb" }}>₹{p.price}</span>
                    <span style={{ fontSize: 11, color: "#ef4444" }}>cost ₹{p.cost}</span>
                    <span style={{ fontSize: 11, color: "#16a34a", fontWeight: 700 }}>
                      +₹{p.price - p.cost} ({Math.round(((p.price - p.cost) / p.price) * 100)}%)
                    </span>
                  </div>
                </div>
                <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                  <button onClick={() => edit(p)} style={{ width: 36, height: 36, borderRadius: 8, border: "1px solid #e5e0d8", background: "#fff", cursor: "pointer", color: "#2563eb", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Icon name="edit" size={14} />
                  </button>
                  <button onClick={() => del(p.id)} style={{ width: 36, height: 36, borderRadius: 8, border: "1px solid #e5e0d8", background: "#fff", cursor: "pointer", color: "#ef4444", display: "flex", alignItems: "center", justifyContent: "center" }}>
